...
import { MapPin, Clock, Calendar, DollarSign, Home, Utensils, Car } from "lucide-react"
  ...
  return (
    <Card className="bg-card border-border hover:ring-2 hover:ring-ring transition-all duration-200">
      <CardHeader className="pb-3">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 sm:gap-4">
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-base lg:text-lg text-foreground line-clamp-2">{job.title}</h3>
            <p className="text-muted-foreground font-medium text-sm lg:text-base">{job.company}</p>
          </div>
      ...
      <CardContent className="space-y-4">
        ...
        {/* Benefits Icons */}
        {(job.benefits.accommodation || job.benefits.meals || job.benefits.transport) && (
          <div className="flex flex-wrap gap-2">
            {job.benefits.accommodation && (
              <div className="flex items-center gap-1 text-xs text-accent-foreground bg-secondary px-2 py-1 rounded">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 pt-2">
          <span className="text-xs lg:text-sm text-muted-foreground">{formatPostedDate(job.postedDate)}</span>
          <Link href={`/jobs/${job.id}`} className="w-full sm:w-auto">
            <Button className="w-full sm:w-auto bg-primary text-primary-foreground hover:bg-primary/90" size="sm">
              View Details
            </Button>
          </Link>
        </div>
